<x-pasien-layout id="add-dokter-layout" title="Ubah Konsultasi" maxWidth="md">
            ...
            <div class="space-y-4 max-w-xl ">
                ...
                <select
                    ...
                    @foreach ($dokters as $dokter)
                        <option value="{{ $dokter->id }}" @if ($dokter->id == $konsultasi->id_user_dokter) selected @endif>
                            {{ $dokter->nama }}
                        </option>
                    @endforeach
                </select>
                <x-text-input label='Subjek' id="subjek" placeholder="Masukkan subjek"
                    value="{{ $konsultasi->subjek }}" />
