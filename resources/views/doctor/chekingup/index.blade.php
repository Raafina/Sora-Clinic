<x-doctor-layout>
    ...
    <div class="flex justify-between py-4">
        <x-search-input route="{{ route('doctor.chekingup.index') }}" placeholder="Cari berdasarkan nama pasien" />
    </div>
    ...
    <x-table :headers="['No', 'Nama Pasien', 'No Antrian', 'Keluhan', 'Aksi']">
        @forelse ($checkupAppointments as $checkupAppointment)
            <x-table-row>
                ...
                <x-table-cell isHeader="true">{{ $checkupAppointment->patient->name }}</x-table-cell>
                <x-table-cell>
                    <p class="bg-primary px-2 py-1 w-fit text-white rounded-lg">{{ $checkupAppointment->queue_number }}
                    </p>
                </x-table-cell>
                <x-table-cell>{{ $checkupAppointment->complaint }}</x-table-cell>
                <x-table-cell>
